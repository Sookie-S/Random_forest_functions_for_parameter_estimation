...
def run_untuned_RF(X, y, testSize, n_opt, genes_alias, genes_nature, outdir, top_n_features=20):
    """
    Run an untuned Random Forest classifier and analyse its results, including Precision-Recall curve, F1-Score, ROC curve, and AUC.

    Parameters:
        X (pd.DataFrame): Feature data.
        y (pd.Series): Target labels.
        testSize (float): Proportion of the test set.
    ...
    rf_classifier = RandomForestClassifier(
        n_estimators=n_opt, random_state=42
    )
